...
// TODO(demax) : ts_ is not correct,should be the time interval between two consecutive imu data
void HPSINS::ConeScullCompensation() {
    ...
    dvbm_ += cm.cross(imus_.at(num_samples_-1).Acce()*ts_) + sm.cross(imus_.at(num_samples_-1).Gyro()*ts_);
